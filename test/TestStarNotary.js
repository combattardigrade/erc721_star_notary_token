...
it('can add the star name and star symbol properly', async() => {
    ...
    let testName = 'Star Notary Token';
    let testSymbol = 'SNT';
    let instance = await StarNotary.deployed();        
    let starName = await instance.name.call();
    let starSymbol = await instance.symbol.call();
    assert.equal(starName,testName);
    assert.equal(testSymbol,starSymbol);
});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let starName = 'testName';
    let starTokenId = 9;
    let instance = await StarNotary.deployed();
    await instance.createStar(starName,starTokenId,{from: accounts[0]});
    assert.equal(await instance.lookUptokenIdToStarInfo(starTokenId), starName);
});